    ...
    def __read_data_from_dir(self, file_path, column_name, 
        ...
        if list_paths_txt:
            for path_txt in list_paths_txt:
                lines = self.__read_data_from_txt(path_txt, delimiter)
                array_lines.extend(lines)
                array_lines_dict.append( {'data': lines, 'filename': path_txt} )
            
        if list_paths_csv:
            for path_csv in list_paths_csv:
                lines = self.__read_data_from_csv(path_csv, column_name, header, nrows)
                array_lines.extend(lines)
                array_lines_dict.append( {'data': lines, 'filename': path_csv} )
        
        return array_lines_dict
